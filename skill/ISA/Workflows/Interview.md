...
Not for Scaffold's ambiguity check: Scaffold asks its own ≤3 questions (Scaffold Step 3.5 § Question mechanics) and records them as `interview_invoked`. Those never run through this workflow and never set `interview_ran`.
...
| **Vision** | "When this is done, what does the user feel?" / "What would delight look like for this work?" / "If a friend used it, what would make them tell another friend?" |
...
When the first question is asked, set frontmatter `interview_ran: <ISO-8601>` — this is what the E5 gate checks, and it means "the Interview was offered", so declining it can never make an ISA unclosable. (It is distinct from `interview_invoked`, which only records that Scaffold's ambiguity check asked questions; those 3 questions do not count as the E5 Interview.) The outcome is recorded separately: when the interview ends with the required sections answered, set `context_sufficient: true`; if it ended on skips, `proceed`, or abandonment, leave or set it `false` and add a Decisions row naming what went unanswered.
...
Run `Workflows/CheckCompleteness.md` with `moment: articulation` after the interview. Surface any remaining gaps but do not block — the user can iterate later.
