...
- The work loop at OBSERVE on a project that has no `<project>/ISA.md` and the task is non-trivial: `Skill("ISA", "seed <project-path>")`
...
Lift the README's headline pitch + any "What it feels like to use this" prose. If the README is dry, leave Vision as a stub with a comment: `<!-- TODO: author Vision — what would delight look like for this project? -->`
...
These are author-driven. Leave them out (empty sections never appear). Add a final TODO note in Decisions: "Seed-generated draft. Run `Skill('ISA', 'interview me on <path>')` to fill Principles, refine Vision and Goal, audit Criteria."
...
started: <ISO-8601>
updated: <ISO-8601>
context_sufficient: false   # a seeded draft is unreviewed by definition; Interview flips it
...
Emit `review_required: true` in the report. Seed is a **draft** — the human-author pass is mandatory before the ISA is treated as authoritative. The user's first task on the project should run Interview to refine.
